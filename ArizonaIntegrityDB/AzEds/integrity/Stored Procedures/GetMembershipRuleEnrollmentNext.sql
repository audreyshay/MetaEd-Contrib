/********************************************************************************************************
**Procedure Name:
**      [integrity].[GetMembershipRuleEnrollmentNext]
**
**Author:
**      Amit Verma
**
**Description:  
**	Get nextMembershipData for integrity membership rule processing  
**               
**Input:
**
**Output:
...
**  Amit Verma  01/12/2016  Added the view to get Next Enrollment records
**  PSharma		01/31/2017  Changed the order of selection from Descending to Ascending
...
**
**********************************************************************************************************/
CREATE PROCEDURE [integrity].[GetMembershipRuleEnrollmentNext]
	@ExecutionId AS int,
...
BEGIN 
      SET NOCOUNT ON;
		  ...
		  FROM 
		  -- Get student enrollment records, along with next entry date
		  (SELECT LEAD(sa.EntryDate) OVER (PARTITION BY sa.fiscalyear, sa.StudentUSI, sa.schoolid ORDER BY sa.EntryDate ASC) AS NextEntryDate , 
				sa.fiscalyear, 
				sa.StudentUSI, 
				sa.schoolid, 
				sa.EntryDate,
		  ...
		  INNER JOIN leadata.StudentSchoolAssociation ssa
			 ON ssa.FiscalYear = ssadup.FiscalYear
				...
				AND ssa.EntryDate = ssadup.NextEntryDate
