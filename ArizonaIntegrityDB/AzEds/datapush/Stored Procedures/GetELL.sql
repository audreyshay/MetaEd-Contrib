/***********************************************************************************************************************        
**  Name			:[datapush].[GetELL]
**        
**  Description     : Proc used by SSIS packages to pull ELL Data from ace.MembershipSummay Table
**
**  Author			: Amit Verma      
**  Date			: 2016-04-05        
**        
...
***	Amit Verma		06/27/2016	XGLT.ID AS RefGradeID instead of XGLT.XSDID
...
***
************************************************************************************************************************/
-- EXEC [datapush].[GetELL] 2015, 3344

CREATE PROCEDURE [datapush].[GetELL]
@FiscalYear INT,
@ExecutionId INT
AS 
...
SELECT ACE.FiscalYear
	  ...
	  ,XPDB.ID														AS PreDefBilingualID  --- Cross Reference Table
	  ,ACE.LimitedMembershipDays                                    AS ELLLimitedMembershipDays
	  ,ACE.LimitedAverageDailyMembership							AS LepAddOnCnt
...
FROM  @MembershipSummaryWithReportingPeriod ACE
	  ...
	  JOIN config.ReportingPeriod RP
	    ON CASE WHEN ACE.ReportingPeriodId = 5 THEN 2
